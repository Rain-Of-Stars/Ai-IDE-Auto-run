...
- **多种捕获后端**：支持传统屏幕截取和现代窗口级捕获，适应不同应用场景
...
# Windows 窗口级截屏（WGC）相关（可选）
windows-capture-python>=1.0.0
winrt-runtime>=2.0.0
```
...
本项目支持两种主要的屏幕捕获方式，每种方式都有其特定的优势和适用场景：

### 1. 传统屏幕截取（Screen Capture）
**技术实现**：基于 `mss` 库进行全屏或指定区域截屏

**特点**：
- ✅ **兼容性强**：适用于所有类型的应用程序和窗口
- ✅ **稳定可靠**：成熟的技术方案，很少出现兼容性问题
- ✅ **配置简单**：只需指定屏幕区域或监视器索引
- ❌ **性能开销**：需要截取整个屏幕区域，数据量较大
- ❌ **遮挡影响**：当目标窗口被其他窗口遮挡时无法正常工作
- ❌ **最小化限制**：无法捕获最小化的窗口内容
...
### 2. 窗口级捕获（Window Capture）
**技术实现**：优先使用 Windows Graphics Capture (WGC)，回退到 PrintWindow API

**特点**：
- ✅ **高性能**：直接捕获目标窗口，无需处理整个屏幕
- ✅ **抗遮挡**：即使窗口被其他窗口遮挡也能正常捕获
- ✅ **最小化支持**：可以捕获最小化窗口的内容（自动恢复但不激活）
- ✅ **精确定位**：直接针对特定窗口，避免误检测
- ✅ **现代化**：使用 Windows 10/11 的现代图形API
- ❌ **依赖性强**：需要额外的WGC库支持
- ❌ **兼容性限制**：某些特殊应用可能不支持
- ❌ **配置复杂**：需要获取窗口句柄或窗口标题

**适用场景**：
- 针对特定应用程序的自动化（如 Trae IDE、Windsurf、Cursor等）
- 需要在窗口被遮挡时继续工作
- 性能敏感的场景
- 现代 Electron 应用程序
...
### 3. 自动选择模式（Auto）
程序会根据配置自动选择最适合的捕获方式：
- 如果配置了有效的窗口句柄或窗口标题，优先使用窗口级捕获
- 如果窗口级捕获初始化失败，自动回退到屏幕截取
- 提供最佳的兼容性和性能平衡

### 配置建议

**推荐配置流程**：
1. **首次使用**：建议先使用传统屏幕截取模式，确保基本功能正常
2. **性能优化**：在确定目标应用后，切换到窗口级捕获以获得更好性能
3. **特殊场景**：对于经常最小化的应用，必须使用窗口级捕获

**Electron应用优化**：
对于 Electron 应用（如 Trae IDE、VS Code等），建议：
- 启用窗口级捕获
- 在应用启动参数中添加 `--disable-features=CalculateNativeWinOcclusion`
- 在 Electron 应用中关闭 `backgroundThrottling`
...
#### 捕获后端设置
- **捕获模式**：选择捕获方式（`screen`=屏幕截取，`window`=窗口捕获，`auto`=自动选择）
- **目标窗口句柄**：窗口级捕获的目标窗口HWND（十进制数字）
- **目标窗口标题**：通过窗口标题自动查找目标窗口
- **窗口标题部分匹配**：是否允许窗口标题的部分匹配
- **捕获帧率上限**：窗口捕获的最大FPS（1-60）
- **捕获超时时间**：单次捕获的超时时间（毫秒）
...
#### wgc_capture.py
- Windows Graphics Capture (WGC) 窗口级捕获实现
- 支持WGC优先，PrintWindow API回退的双重保障
- 处理最小化窗口的无激活恢复
- 提供高性能的窗口内容捕获
- 包含窗口查找和Electron应用检测辅助功能
...
### 诊断工具
- **diagnose_multiscreen_click.py**：诊断多屏幕点击问题，分析屏幕设置和坐标差异
- **fix_multiscreen_config.py**：自动修复多屏幕配置，解决坐标偏移问题
- **debug_coordinates.py**：调试坐标计算问题，验证点击位置准确性
...
# 窗口级捕获演示和测试
python scripts/demo_window_capture_match.py
```
...
- **WGC库安装失败**：
  ...
  ```
- **窗口句柄获取失败**：
  - 使用任务管理器或Process Explorer查看窗口信息
  - 尝试使用窗口标题自动查找
  - 确保目标应用程序正在运行
- **Electron应用捕获异常**：
  - 添加启动参数：`--disable-features=CalculateNativeWinOcclusion`
  - 在应用设置中关闭后台节流（backgroundThrottling）
  - 增加捕获超时时间
- **最小化窗口无法捕获**：
  - 启用"自动恢复最小化窗口"选项
  - 检查窗口是否支持无激活恢复
...
#### 6. 捕获后端选择建议
- **首次使用**：推荐使用`auto`模式，让程序自动选择
- **性能优先**：使用`window`模式，配置目标窗口
- **兼容性优先**：使用`screen`模式，传统屏幕截取
- **调试模式**：启用调试日志，查看后端切换过程
