...
class WGCCaptureBackend:
    ...
    # ----------------- 内部工具函数：处理WGC帧步幅 -----------------
    def _extract_bgra(self, frame: Any) -> Optional[np.ndarray]:
        """从 windows-capture 的帧对象中提取连续的 BGRA 图像。
        关键点：显卡纹理到CPU映射后每行存在对齐填充（RowPitch/stride），
        若直接按 w*h*4 重塑会导致每行错位，画面呈现斜切/扭曲。

        本函数尽可能兼容不同库字段：stride/row_stride/row_pitch/pitch。
        返回形状 (h, w, 4) 的 np.uint8 BGRA 数组；失败返回 None。
        """
        try:
            w = getattr(frame, 'width', None)
            h = getattr(frame, 'height', None)
            buf = getattr(frame, 'buffer', None)
            if not (w and h and buf is not None):
                return None

            # 可能存在的步幅字段名（按优先级尝试）
            stride = (
                getattr(frame, 'stride', None)
                or getattr(frame, 'row_stride', None)
                or getattr(frame, 'row_pitch', None)
                or getattr(frame, 'pitch', None)
            )

            # 将底层缓冲区转为一维视图（不复制）
            arr = np.frombuffer(buf, dtype=np.uint8)
            row_bytes = int(w) * 4  # BGRA 每像素4字节

            if stride is None or int(stride) == row_bytes:
                # 无对齐填充或与行字节一致，可直接重塑
                need = int(h) * row_bytes
                if arr.size < need:
                    return None
                return arr[:need].reshape((int(h), int(w), 4))
            ...
            for y in range(int(h)):
                ...
                if end > arr.size:
                    return None
                out[y, :, :] = arr[start:end].reshape((1, int(w), 4))
            return out
        except Exception:
            return None
    ...
    def _on_frame(self, frame: Any) -> None:
        ...
        try:
            with self._lock:
                # 统一通过 _extract_bgra 处理步幅
                bgra = self._extract_bgra(frame)
                if bgra is not None:
        ...
        try:
            ...
            if get_fn:
                ...
                if isinstance(data, np.ndarray):
                    ...
                    return data
                # 兼容 windows-capture 返回的帧对象
                bgra = self._extract_bgra(data)
                if bgra is not None:
...
def find_window_by_process(proc: str, partial_match: bool = True) -> Optional[int]:
    """根据进程名或完整路径查找窗口HWND。
    - 支持传入进程文件名（如 Code.exe）或完整路径；
    - 当 partial_match=True 时，允许在进程文件名或完整路径中做子串匹配；
    - 仅返回第一个可见顶层窗口的句柄。
    """
    if not proc:
        return None
    ...
    log = get_logger()
    target = os.path.normcase(proc).replace('/', '\\')
        ...
        try:
            ...
            try:
                ...
                if psapi.GetModuleFileNameExW(h, None, buf, 1024):
                    ...
                    if partial_match:
                        ok = (target_lower in full_lower) or (target_lower in base_lower)
                    else:
                        ok = (target_lower == full_lower) or (target_lower == base_lower)
                    if ok:
                        found_hwnd = hwnd
                        return False
            finally:
            ...
            # 出错不中断枚举
            try:
                log.debug(f"find_window_by_process: 跳过窗口 {hwnd}: {e}")
            except Exception:
                pass
            return True

    user32.EnumWindows(enum_proc, 0)
    return int(found_hwnd) if found_hwnd else None
